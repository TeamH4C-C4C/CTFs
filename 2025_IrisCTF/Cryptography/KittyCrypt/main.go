...
func getKeys(length int) []int { // length = 46
	var keys = []int{}
	keyFileName := fmt.Sprintf("keys_%d.json", length)	// keys_46.json

	file, err := os.Open(keyFileName)
	if err != nil {

		for i := 0; i < length; i++ {
			num, _ := rand.Int(rand.Reader, big.NewInt(60000))	// 0 ~ 59,999
